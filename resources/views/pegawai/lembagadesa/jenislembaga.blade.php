...
@section('link')
    {{ route('pegawai.lembagadesa.jenislembaga') }}
@endsection
...
@section('content')
    ...
    @include('pegawai.lembagadesa.modals._jenislembaga')
@endsection
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#order-table').DataTable({
                ...
                ajax: {
                    url: "{{ route('pegawai.lembagadesa.jenislembaga') }}",
                },
                columns: [
                {
                    data: 'DT_RowIndex',
            ...
            $('#form_jenislembaga').on('submit', function (event) {
                ...
                if ($('#action').val() == 'edit') {
                    url = "{{ route('pegawai.lembagadesa.jenislembaga-update') }}";
            ...
            $(document).on('click', '.edit', function () {
                var id = $(this).attr('id');
                $.ajax({
                    url: '/pegawai/lembagadesa/jenislembaga/'+id,
                    dataType: 'JSON',
                    success: function (data) {
                        $('#action').val('edit');
            ...
            $('#ok_button').click(function () {
                $.ajax({
                    url: '/pegawai/lembagadesa/jenislembaga/hapus/'+user_id,
                    beforeSend: function () {
