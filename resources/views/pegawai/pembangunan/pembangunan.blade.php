...
@section('describ')
    Ini adalah halaman Pembangunan untuk pegawai
@endsection
...
@section('link')
    {{ route('pegawai.pembangunan.pembangunan') }}
@endsection
...
@section('content')
    ...
    @include('pegawai.pembangunan.modals._pembangunan')
@endsection
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#order-table').DataTable({
                ...
                ajax: {
                    url: "{{ route('pegawai.pembangunan.pembangunan') }}",
                },
                columns: [
                {
                    data: 'DT_RowIndex',
            ...
            $('#form-pembangunan').on('submit', function (event) {
                ...
                if ($('#action').val() == 'edit') {
                    url = "{{ route('pegawai.pembangunan.pembangunan-update') }}";
            ...
            $(document).on('click', '.edit', function () {
                var id = $(this).attr('id');
                $.ajax({
                    url: '/pegawai/pembangunan/pembangunan/'+id,
                    dataType: 'JSON',
                    success: function (data) {
                        console.log(data);
                        $('#action').val('edit');
            ...
            $('#ok_button').click(function () {
                $.ajax({
                    url: '/pegawai/pembangunan/pembangunan/hapus/'+user_id,
                    beforeSend: function () {
