    ...
    <script>
        $(document).ready(function () {
            $('#order-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('pegawai.peristiwa.kelahiran') }}",
                },
                columns: [
                {
                    data: 'DT_RowIndex',
            ...
            if ($('#button').hasClass('add')) {
                url = "{{ route('pegawai.kelahiran.add') }}";
                text = "Data berhasil ditambahkan";
            }

            if ($('#button').hasClass('edit')) {
                url = "{{ route('pegawai.kelahiran.update') }}";
        ...
        $(document).on('click', '.edit', function() {
            ...
            $.ajax({
                url: '/pegawai/peristiwa/kelahiran/edit/' + id,
                dataType: 'JSON',
                success: function(data) {
            ...
            $('.delete_btn').click(function () {
                $.ajax({
                    url: '/pegawai/peristiwa/kelahiran/hapus/'+delete_id,
                    beforeSend: function () {
