    ...
    <script>
        $(document).ready(function () {
            ...
            $('#order-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('pegawai.peristiwa.kematian') }}",
                },
                columns: [
                {
                    data: 'DT_RowIndex',
        ...
        $('#deathdate').dateDropper({
            ...
            if ($('#button').hasClass('add')) {
                url = "{{ route('pegawai.kematian.add') }}";
                text = "Data berhasil ditambahkan";
            }

            if ($('#button').hasClass('edit')) {
                url = "{{ route('pegawai.kematian.update') }}";
        ...
        $(document).on('click', '.edit', function() {
            ...
            $.ajax({
                url: '/pegawai/peristiwa/kematian/edit/' + id,
                dataType: 'JSON',
                success: function(data) {
            ...
            $('.delete_btn').click(function () {
                $.ajax({
                    url: '/pegawai/peristiwa/kematian/hapus/'+delete_id,
                    beforeSend: function () {
