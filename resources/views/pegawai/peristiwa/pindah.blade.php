    ...
    <script>
        $(document).ready(function () {
            $('#order-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('pegawai.peristiwa.pindah') }}",
                },
                columns: [
                {
                    data: 'DT_RowIndex',
        ...
        $('#movedate').dateDropper({
            ...
            if ($('#button').hasClass('add')) {
                url = "{{ route('pegawai.pindah.add') }}";
                text = "Data berhasil ditambahkan";
            }

            if ($('#button').hasClass('edit')) {
                url = "{{ route('pegawai.pindah.update') }}";
        ...
        $(document).on('click', '.edit', function() {
            ...
            $.ajax({
                url: '/pegawai/peristiwa/pindah/edit/' + id,
                dataType: 'JSON',
                success: function(data) {
            ...
            $('.delete_btn').click(function () {
                $.ajax({
                    url: '/pegawai/peristiwa/pindah/hapus/'+delete_id,
                    beforeSend: function () {
