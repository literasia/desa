...
@section('describ')
    Ini adalah halaman Jenis Jenis Pembangunan untuk admin
@endsection
...
@section('link')
    {{ route('admin.pembangunan.jenispembangunan') }}
@endsection
    ...
    <script>
        $(document).ready(function () {
            $('#order-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('admin.pembangunan.jenispembangunan') }}",
                },
                columns: [
                {
                    data: 'DT_RowIndex',
            ...
            $('#jenispembangunan').on('submit', function (event) {
                ...
                if ($('#action').val() == 'edit') {
                    url = "{{ route('admin.pembangunan.jenispembangunan-update') }}";
                }
            ...
            $(document).on('click', '.edit', function () {
                var id = $(this).attr('id');
                $.ajax({
                    url: '/admin/pembangunan/jenispembangunan/'+id,
                    dataType: 'JSON',
                    success: function (data) {
            ...
            $('#ok_button').click(function () {
                $.ajax({
                    url: '/admin/pembangunan/jenispembangunan/hapus/'+user_id,
                    beforeSend: function () {
